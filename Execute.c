...
int main() {
    ...
    Standard_scaler* scaler = new_scaler("Standard_scaler");
    ...
    scaler_save("D:\\Data\\code_doc\\AI_model_building\\scaler_h5.txt", scaler, "Standard_scaler");
    ...
    //Optimizer* opt = new_optimizer("Adagrad", 0.09, Nan, Nan, Nan, Nan, Nan, 1e-8, 1e-2);
    ...
    model_fit(model, ds, 300, 32, 0.2, estop);

    model_evaluate(model, test_ds);
    model_save("D:\\Data\\code_doc\\AI_model_building\\model_h5.txt", model);
