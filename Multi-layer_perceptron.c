...
static void free_activated_data(MLP* model, int layer, int samples) {
    if (!model->a || !model->a[layer] || !model->z[layer]) return ;
...
void model_fit(MLP* model, Dataset_2* data, int epochs, int batch_size, float validation_split, void* call_backs) {
    ...
    for (i = 1; i <= epochs; i++) {
        ...
        printf("Epoch %d/%d\n%d/%d [==============================] - %.2fs - loss: %.4f - %s: %.4f", 
            i, epochs, loop + h, loop + h, time, loss, Metrics[model->compiler->metrics_type], metrics);
        if (validation_split) {
...
void model_save(const char* file_name, MLP* model) {
    ...
    int i, j, k;
    fprintf(file, "Multi-layer_perceptron\n%d\n", model->num_layers);
    for (i = 0; i < model->num_layers; i++) fprintf(file, "%d ", model->num_units[i]);
    fprintf(file, "\n");
    for (i = 0; i < model->num_layers; i++) fprintf(file, "%d ", model->activation[i]);
    fprintf(file, "\n");
    for (i = 0; i <= model->drop_layer[0]; i++) fprintf(file, "%d ", model->drop_layer[i]);
    fprintf(file, "\n");
    for (i = 0; i < model->drop_layer[0]; i++) fprintf(file, "%f ", model->dropout[i]->drop);
    fprintf(file, "\n");
    fprintf(file, "%d %d\n", model->compiler->loss_type, model->compiler->metrics_type);
    for (i = 0; i < model->num_layers; i++) {
        for (j = 0; j < model->w[i]->col; j++) {
            for (k = 0; k < model->w[i]->row; k++) fprintf(file, "%.20f ", model->w[i]->weights[k][j]);
            fprintf(file, "%.20f\n", model->w[i]->bias[j]);
        }
    }
    fclose(file);
}
MLP* load_model(const char* file_name) {
    FILE* file = fopen(file_name, "r");
    if (!file) {
        printf("Error: Cannot open file %s !!", file_name);
        return NULL;
    }
    char* s = (char*)malloc(100000* sizeof(char)), * token;
    if (!s) {
        printf("Error: Out of memory for mallocing !!");
        return NULL;
    }
    int i, j, k;
    fgets(s, 100000, file);
    s[strcspn(s, "\n")] = '\0';
    if (strcmp(s, "Multi-layer_perceptron")) {
        printf("Warning: Inappropriated type of model !!");
        return NULL;
    }
    MLP* model = (MLP*)calloc(1, sizeof(MLP));
    model->cur_batch_size = 0;
    fscanf(file, "%d", &(model->num_layers));
    model->z = (float***)calloc(model->num_layers, sizeof(float**));
    model->a = (float***)calloc(model->num_layers, sizeof(float**));
    while ((j = fgetc(file)) != '\n' && j != EOF);
    model->num_units = (int*)malloc(model->num_layers* sizeof(int));
    for (i = 0; i < model->num_layers; i++) fscanf(file, "%d", &(model->num_units[i]));

    while ((j = fgetc(file)) != '\n' && j != EOF);
    model->activation = (int*)malloc(model->num_layers* sizeof(int));
    for (i = 0; i < model->num_layers; i++) fscanf(file, "%d", &(model->activation[i]));

    while ((j = fgetc(file)) != '\n' && j != EOF);
    fscanf(file, "%d", &k);
    model->drop_layer = (int*)malloc((k + 1)* sizeof(int));
    model->drop_layer[0] = k;
    for (i = 1; i <= k; i++) fscanf(file, "%d", &(model->drop_layer[i]));

    while ((j = fgetc(file)) != '\n' && j != EOF);
    model->dropout = (Dropout**)malloc(k* sizeof(Dropout*));
    for (i = 0; i < k; i++) {
        model->dropout[i] = (Dropout*)malloc(sizeof(Dropout));
        fscanf(file, "%f", &(model->dropout[i]->drop));
    }

    while ((j = fgetc(file)) != '\n' && j != EOF);
    model->compiler = (Model_Compiler*)malloc(sizeof(Model_Compiler));
    fscanf(file, "%d %d", &(model->compiler->loss_type), &(model->compiler->metrics_type));

    while ((j = fgetc(file)) != '\n' && j != EOF);
    model->w = (Weights**)malloc(model->num_layers* sizeof(Weights*));
    fgets(s, 100000, file);
    for (i = 0, k = 0; s[i] != '\n'; i++) if (s[i] == ' ') k++;
    s[i] = '\0';
    model->w[0] = init_weights(k, model->num_units[0], 0);
    token = strtok(s, " ");
    for (i = 0; i < k; i++) {
        sscanf(token, "%f", &(model->w[0]->weights[i][0]));
        token = strtok(NULL, " ");
    }
    sscanf(token, "%f", &(model->w[0]->bias[0]));
    for (j = 1; j < model->num_units[0]; j++) {
        fgets(s, 100000, file);
        token = strtok(s, " ");
        for (i = 0; i < k; i++) {
            sscanf(token, "%f", &(model->w[0]->weights[i][j]));
            token = strtok(NULL, " ");
        }
        sscanf(token, "%f", &(model->w[0]->bias[j]));
    }

    for (k = 1; k < model->num_layers; k++) {
        model->w[k] = init_weights(model->num_units[k - 1], model->num_units[k], 0);
        for (j = 0; j < model->num_units[k]; j++) {
            fgets(s, 100000, file);
            token = strtok(s, " ");
            for (i = 0; i < model->num_units[k - 1]; i++) {
                sscanf(token, "%f", &(model->w[k]->weights[i][j]));
                token = strtok(NULL, " ");
            }
            sscanf(token, "%f", &(model->w[k]->bias[j]));
        }
    }
    free(s);
    fclose(file);
